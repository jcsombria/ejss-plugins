...
//  private  static final char E_CODE              = (char)254;
  ...
  static public boolean isKeyword (String token) { // Added by Paco
    try { Double.parseDouble(token); return true;} // It's a valid constant
    catch (Exception _exc) {}; // Do nothing
    for (int i = 0; i < NO_CONST; i++) if (token.equals(constname[i])) return true;
    for (int i = 0; i < NO_FUNCS; i++) if (token.equals(funcname[i])) return true;
    for (int i = 0; i < NO_EXT_FUNCS; i++) if (token.equals(extfunc[i])) return true;
    for (int i = 0; i < NO_FUNCSNOPARAM; i++) if (token.equals(funcnameNoParam[i])) return true;
    return false;
  }
  ...
  static public String[] getVariableList (String _expression) { // Added by Paco
    java.util.Vector<String> varlist = ParserSuryono.getVariableList(_expression,new java.util.Vector<String>());
    if (varlist.size()<1) return new String[0];
  ...
  static public Vector<String> getVariableList (String _expression, Vector<String> varlist) { // Added by Paco
    ...
    while (tkn.hasMoreTokens()) {
      ...
      if (!varlist.contains(token)) varlist.add(token);
    }
    return varlist;
  }
 ...
 *----------------------------------------------------------------------------------------*/
  ...
  private void scanNumber() throws ParserException {
    ...
    if (character == 'e' || character=='E') {  // Paco added 'E'
      numstr += 'e'; getNextCharacter();       // Paco changes character to 'e'
...
*/
  ...
  private void relationLevel() throws ParserException {
    ...
    switch (character) {
      case '!':
        code = NOT_CODE;  // Paco changed this
        getNextCharacter();
        if (character == '=') code = NOT_EQUAL; else position--;
        break;
      case '=':
        code = EQUAL;  // Paco changed this
        getNextCharacter();
        if (character == '=') code = EQUAL; else position--;
        break;
      case '<': code = LESS_THAN;
                getNextCharacter();
   ...
   * @param  _function  the function index
...
*/
  ...
  private double builtInFunction(int _function, double parameter) {
    switch (_function) {
      case  0: return Math.abs(parameter);
      case  1: return Math.acos(parameter);
      case  2: return Math.asin(parameter);
      case  3: return Math.atan(parameter);
      case  4: return Math.ceil(parameter);

      case  5: return Math.cos(parameter);
      case  6: return Math.exp(parameter);
      case  7: return Math.floor(parameter);
      case  8: return Math.log(parameter);
      case  9: return Math.rint(parameter);

      case 10: return Math.round(parameter);
      case 11: return Math.sin(parameter);
      case 12: return Math.sqrt(parameter);
      case 13: return Math.tan(parameter);
      case 14: return Math.toDegrees(parameter);

      case 15: return Math.toRadians(parameter);
  ...
  private double builtInFunctionNoParam(int _function) {  // Paco
    switch (_function) {
...
//  { "Math.atan2", "Math.IEEEremainder", "Math.max", "Math.min", "Math.pow"};
  ...
  private double builtInExtFunction(int _function, double param1, double param2) {
    switch (_function) {
      case  0: return Math.atan2(param1,param2);
      case  1: return Math.IEEEremainder(param1,param2);
      case  2: return Math.max(param1,param2);
      case  3: return Math.min(param1,param2);
      case  4: return Math.pow(param1,param2);
  ...
  private double evaluateSubFunction() {
    ...
    char   code;
      ...
      while (true) {
      try {
        if(code_pointer==codeLength) return stack[0]; //added by W. Christian.  Do not use doing an Exception!
      ...
      try {
        switch (code) {
          ...
          */
          ...
          case JUMP_CODE     : destination = code_pointer + postfix_code.charAt(code_pointer++);
...
//          case E_CODE        : stack[++stack_pointer] = Math.E; break; Paco
          ...
          default: if (code >= REF_OFFSET)
                   ...
                   if (code >= FUNC_OFFSET) {
                     stack[stack_pointer] =
                     builtInFunction(code-FUNC_OFFSET, stack[stack_pointer]);
                   }
                   else
                   if (code >= CONST_OFFSET) { // Added by Paco for constants
