...
//		outStream = outContainer.addNewStream(codec); // ver 5.4
		...
		outStreamCoder.setCodec(codec);	 // ver 3.4 only--remove for ver 5.4
	  ...
	  if(dim!=null) {
		...
		if (typicalName.endsWith(".avi") && frameRate.getDenominator()>65535) { // maximum limit for AVI container //$NON-NLS-1$
			double fps = 1000/frameDuration;
			int denom = 9000; // pig test this
...
//    IMetaData params = IMetaData.make(); // ver 5.4
//		if (outStreamCoder.open(params, params)<0) {
