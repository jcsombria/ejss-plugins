...
public class Radau5Adaptive extends Radau5Light {
    protected ERROR error_code=ERROR.NO_ERROR;
...
//        return new IRKSimplifiedNewton(algEqn, laeSolver);
    ...
    protected void estimateNewtonInitialValue(double [][] initialvalue){
        double factor = currentStepSize / actualStepSize; // taken step size in denominator indeed
        for (int j = 0; j < 3; j++)
    ...
    public double doStep() {
        ...
        do {
            currentStepSize = actualStepSize;
            try{
                double newtonConvergenceRate = aeSolver.resolve();
                error = estimateError();
                // TODO: ((MyNewton)aeSolver).getnIter()!!!
                actualStepSize = limitStepSize(estimateStepSize(error, ((MyNewton)aeSolver).getnIter(), 7));
                if (error < 1) {
                 // TODO: 0.001 is a parameter
                    if ((Math.abs((actualStepSize / currentStepSize)-1.1) <= 0.1)&&(newtonConvergenceRate <= 0.001)){
                      actualStepSize = currentStepSize; // it's a very cheap to stay stepsize as is.
                    }
                ...
                actualStepSize = currentStepSize / 2;
                aeSolver.restart(jacobianAge > 0);
                nRejected++;
...
// TODO: "hhfac = 0.5; && nmSingularMatrix" of original code had been lost on translation !!!
                ...
                actualStepSize = limitStepSize(currentStepSize * 0.8 * Math.pow(qnewt, -1.0 / (4.0 + e.getMaxIterationsAllowed() - 1 - e.getIterationNumber())));
                aeSolver.restart(jacobianAge > 0);
                nRejected++;
                error = 10; // also one loop, no other means
            }
...
// todo: it's unavailable to have state here
            scal[i] = absTol + relTol*Math.abs(state[i]);
...
// TODO: check equivalence with source. Here was SCAL = ..., that is used in Newton && ErrEst
        ...
        if (relTol==0) ((IRKSimplifiedNewton)aeSolver).setFNewton(Double.POSITIVE_INFINITY);
        else ((IRKSimplifiedNewton)aeSolver).setFNewton(Math.max(10 * uround / relTol, Math.min(0.03, Math.pow(relTol, 0.5))));
    }

    public void setTolerances(double[] absTol, double[] relTol) {
      System.err.println("Radau5 does not support the method: setTolerances(double[], double[]);\n");
    }
