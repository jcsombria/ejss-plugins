...
/**
 * Uses interpolation based on extra evaluations appropriated to DoPri5 algorithm
 * 
...
public class Dopri5IntervalData extends ExtraStepsIntervalData {
	...
	public Dopri5IntervalData(double[] aState, double[] bState, double[][] coeffs) {
	  super(aState,bState,coeffs);
	}
	
  @Override
  public double interpolate(double time, int index) {
    double theta = (time-getLeft())/mDeltaTime;
    double theta1 = 1 - theta;
    return mCoeffs[0][index] + theta*(mCoeffs[1][index] + 
        theta1*(mCoeffs[2][index] + theta*(mCoeffs[3][index] + theta1*mCoeffs[4][index])));
    }
  ...
  public double[] interpolate(double time, double[] state, int beginIndex, int length) {
    ...
    for (int index=beginIndex, i=0; i < length; index++, i++) {
      state[i] = mCoeffs[0][index] + theta*(mCoeffs[1][index] + 
          theta1*(mCoeffs[2][index] + theta*(mCoeffs[3][index] + theta1*mCoeffs[4][index])));
    }
    return state;
  }
