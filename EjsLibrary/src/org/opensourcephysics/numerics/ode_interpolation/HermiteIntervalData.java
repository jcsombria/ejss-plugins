...
public class HermiteIntervalData extends IntervalData {
    ...
    super(aState[aState.length-1],bState[bState.length-1]);
	  ...
	  // Copy the states and rates
		...
		mRightState = new double[dimension]; 
		mLeftRate   = new double[dimension]; 
		mRightRate  = new double[dimension]; 
		System.arraycopy(aState,0,mLeftState, 0,dimension);
		System.arraycopy(bState,0,mRightState,0,dimension);
		System.arraycopy(aRate, 0,mLeftRate,  0,dimension);
		System.arraycopy(bRate, 0,mRightRate, 0,dimension);
  ...
  @Override
	public double[] interpolate(double time, double[] state, int beginIndex, int length) {
		hermite((time - getLeft())/mDeltaTime, state, beginIndex, length);
		return state;
	}
