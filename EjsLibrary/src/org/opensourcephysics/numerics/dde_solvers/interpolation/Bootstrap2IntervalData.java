...
//  private boolean mBootStrap2Ready = false;
  private double[] state_bt2 = null, rate_bt21, rate_bt22, bt2_c2, bt2_c3, bt2_c4, bt2_c5;

  public Bootstrap2IntervalData(double[] aState, double[] aRate,double[] bState, double[] bRate, ODE ode, StateMemory stateMemory) {
    super(aState,aRate,bState, bRate, ode, stateMemory);
...
//    if (!mBootStrap2Ready) prepareSecondBootstrap();
  ...
  private void prepareSecondBootstrap() {
    ...
    state_bt2[mTimeIndex] = mLeft + BETA1*mDeltaTime;

    if (mDDE!=null) {
      double[] delays = mDDE.getDelays(state_bt2);
      IntervalData[] intervals = new IntervalData[delays.length];
      for (int i=0; i<delays.length; i++) intervals[i] = mStateMemory.findInterval(state_bt2[mTimeIndex]-delays[i], true);
      mDDE.getRate(state_bt2, intervals, rate_bt21);
    }
    else mODE.getRate(state_bt2, rate_bt21);

    super.bootstrap1(BETA2,state_bt2,0,mTimeIndex);
    state_bt2[mTimeIndex] = mLeft  + BETA2*mDeltaTime;

    if (mDDE!=null) {
      double[] delays = mDDE.getDelays(state_bt2);
      IntervalData[] intervals = new IntervalData[delays.length];
      for (int i=0; i<delays.length; i++) intervals[i] = mStateMemory.findInterval(state_bt2[mTimeIndex]-delays[i], true);
      mDDE.getRate(state_bt2, intervals, rate_bt22);
    }
    else mODE.getRate(state_bt2, rate_bt22);
