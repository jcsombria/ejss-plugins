...
//  }
  ...
  public static String edit (ModelEditor modelEditor, String _property, String _basicType, Component _target, String _value, String _constant) {
    ...
    Vector<String> possibleDomains=new Vector<String>();
...
//      _property = "Variables";
      ...
      if (_basicType!=null) {
        String types = _basicType, domains = "";
        if(_basicType.contains(":")) {
          String[] t = _basicType.split(":");
          types = t[0];
          domains = t[1];
        }
        StringTokenizer typeTkn = new StringTokenizer(types,"| ");
        while (typeTkn.hasMoreTokens()) {
          possibleTypes.add(typeTkn.nextToken());
        }       
        StringTokenizer domainTkn = new StringTokenizer(domains, "| ");
        while (domainTkn.hasMoreTokens()) {
          possibleDomains.add(domainTkn.nextToken());
        }
      }
      if (_constant!=null) {
    ...
    while (tkn.hasMoreTokens()) {
      ...
      boolean typeMatches   = possibleTypes.isEmpty(),
              domainMatches = possibleDomains.isEmpty();
        String[] items = token.split(":");
        String type = items[0].trim(), name = items[1].trim(), domain = items[3].trim();
        ...
        for (String domainDesired : possibleDomains) {
          if(domainDesired.equals(domain)) {
            domainMatches = true;
            break;
          }
        }
        if (typeMatches && domainMatches) {
          ...
          String toReturn = token.substring(start);
          data.add(toReturn);
        }
    }
