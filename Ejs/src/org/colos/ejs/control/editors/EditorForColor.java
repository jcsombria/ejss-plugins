...
//    buttonPanel.add (defaultButton);
    ...
    addColor (colorCenterPanel, "200,220,208,255",    new Color(200,220,208), mouseListener);// FKH 021103
...
//    selectionLabel.setFont(label.getFont().deriveFont (Font.BOLD));
    ...
    for (int r=0; r<=256; r+=64) {
      ...
      for (int g=0; g<=256; g+=64) {
        ...
        for (int b=0; b<=256; b+=64) {
          if (b>255) b = 255;
          addColor (moreColorsPanel, r+","+g+","+b+",255", new Color (r,g,b), mouseListener);
        }
      }
    }
    dialog2 = new JDialog();
  ...
  private static void showReturnColor (Color aColor) {
    ...
    alphaField.setText (""+aColor.getAlpha());
  }
  ...
  private static void readReturnColor () {
    ...
    } catch (Exception exc) { alpha = 255; }
    lastColor = new Color (r,g,b,alpha);
