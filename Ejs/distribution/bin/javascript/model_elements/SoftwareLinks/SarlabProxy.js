function SarlabProxy (secure, ip, basepath, port, experience) {
  ...
  this.typeCredentials = null; // 'key', 'user' or 'token'
  ...
  this.setSarlabInfo(secure === true || secure === 'true', ip, basepath, port, experience, SarlabProxy.prototype.closeScreen);
...
SarlabProxy.prototype.getCredentialString = function (append) {
    if (append) append = '&';
    else append = '';
    if (SarlabProxy.prototype.sessionKey != null) {
        return append + 'keySession=' + SarlabProxy.prototype.sessionKey + '&timeOut=' + this.sessionTime;
    } else if (this.credentials['key'] != null) {
        return append + 'key=' + this.credentials.key;
    }
    return '';
};
...
SarlabProxy.prototype.connectExperience = function(experience, callback) {
  if (this.credentials == null) {
    username = window.prompt("Please, enter your SARLAB user:", "username");
    password = window.prompt("Please, enter your SARLAB password:", "password");
    this.setSarlabCredentials({'username':username, 'password':password});
  }
...
SarlabProxy.prototype.connectSession = function(info, callback) {
    ...
    switch(this.typeCredentials) {
        ...
        case 'user':
            ...
            break;
        /*case 'token':
            myObj.orden.parameters=[this.experience, this.sessionTime, this.typeCredentials, this.credentials.token];
            break;*/
    }

    var wsSarlab = new WebSocket(this.getSarlabPath() + "?idExp=" + this.experience + this.getCredentialString(true));
...
function Transport(url, credentials) {
	...
	this.configurePostRequest = function() {
		...
		this.headers = {
			...
			'Content-Type': 'text/plain' //instead of application/json to avoid OPTIONS preflight request
		};
	};
...
function SarlabWebSocket(command, IP, port, idExp, expTime, user, password, jarPath) {
	...
	ws.onerror = function() {
		...
		a.href = "http://sarlabirs.dia.uned.es/site/download";
		a.target = "_blank";
