...
public class RIPElement extends AbstractModelElement {
  ...
  private JTextField serverText = new JTextField("", 20);
  ...
  protected Component createEditor(String name, Component parentComponent, final ModelElementsCollection collection) {
    ejs = collection.getEJS(); // Store reference to EJS - Needed to retrieve XML configuration
  ...
  protected Component createJilEditor(String name, Component parentComponent, final ModelElementsCollection collection) {
    ...
    mainPanel.setPreferredSize(new Dimension(600,405));
  ...
  private JPanel createServerPanel() {
    ...
    serverPanel.setMinimumSize(new Dimension(600, 405));
    //serverPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 405));
    serverPanel.setPreferredSize(new Dimension(600, 405));
    SpringLayout sl_topPanel = new SpringLayout();
    serverPanel.setLayout(sl_topPanel);
    
    JLabel serverLabel = new JLabel("Server url:");
    JButton testButton = new JButton("Get Experiences");

    sl_topPanel.putConstraint(SpringLayout.NORTH, serverLabel, 7, SpringLayout.NORTH, serverPanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, serverLabel, 6, SpringLayout.WEST, serverPanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, serverText, 6, SpringLayout.EAST, serverLabel);
    sl_topPanel.putConstraint(SpringLayout.EAST, serverText, 6, SpringLayout.EAST, serverPanel);
    serverText.setMinimumSize(new Dimension((int) Math.round(0.9*serverPanel.getWidth()), 0));
    sl_topPanel.putConstraint(SpringLayout.NORTH, testButton, 4, SpringLayout.SOUTH, serverText);
...
//        }
    ...
    sl_topPanel.putConstraint(SpringLayout.EAST, methodsScrollPane, 6, SpringLayout.EAST, serverPanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, methodsScrollPane, 6, SpringLayout.WEST, serverPanel);
  ...
  private JPanel createExperiencePanel() {
    ...
    experiencePanel.setMinimumSize(new Dimension(600, 405));
    //experiencePanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 405));
    experiencePanel.setPreferredSize(new Dimension(600, 405));
    SpringLayout sl_topPanel = new SpringLayout();
...
//    experiencePanel.add(labIdLabel);
    ...
    sl_topPanel.putConstraint(SpringLayout.WEST, expIdLabel, 6, SpringLayout.WEST, experiencePanel);
...
//  sl_topPanel.putConstraint(SpringLayout.VERTICAL_CENTER, labIdLabel, 0, SpringLayout.VERTICAL_CENTER, portText);
    ...
    sl_topPanel.putConstraint(SpringLayout.WEST, labDescriptionLabel, 6, SpringLayout.WEST, experiencePanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, labDescriptionText, 4, SpringLayout.EAST, labDescriptionLabel);
    sl_topPanel.putConstraint(SpringLayout.EAST, labDescriptionText, 6, SpringLayout.EAST, experiencePanel);
...
//  
    ...
    sl_topPanel.putConstraint(SpringLayout.EAST, methodsScrollPane, 6, SpringLayout.EAST, experiencePanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, methodsScrollPane, 6, SpringLayout.WEST, experiencePanel);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
  ...
  private SarlabElement getSarlabElement() {
    for(ModelElementInformation info : ejs.getModelEditor().getElementsEditor().getElements()) {
      try {
        SarlabElement theElement = (SarlabElement)info.getElement();
        return theElement;
  ...
  private String getServerURL() {
    ...
    if (host.isEmpty()) { // Read Sarlab configuration file and build the url
      SarlabElement sarlab = getSarlabElement();
      SarlabConfigurationModel config = sarlab.getConfig();  
      for(Map<String,String> proxy : config.getHtmlLinks()) {
        String type = proxy.get(SarlabConfigurationModel.TYPE);
        if(type.equals("RIP Server")) {  // TO DO: Define as a constant in SarlabConfigurationModel, not hardcoded
          String ip = proxy.get(SarlabConfigurationModel.IP);
          String path = proxy.get(SarlabConfigurationModel.PATH);
          String proto = config.getSecurity() ? "https" : "http";
          String username = config.getUsername();
          String password = config.getPassword();
          if (username.isEmpty() || password.isEmpty()) {
            host = String.format("%s://%s/SARLABV8.0/proxy?url=http://%s/%s",
            ...
            try {
              ...
              System.err.println("[ERROR] Cannot encode username and password");
            }
          }
          return host;
        }
  ...
  private JPanel createVariablesPanel() {
    ...
    variablesPanel.setMinimumSize(new Dimension(600, 405));
    variablesPanel.setPreferredSize(new Dimension(600, 405));
    SpringLayout sl_topPanel = new SpringLayout();
    variablesPanel.setLayout(sl_topPanel);
    JPanel serverVariablesPanel = createServerVariablesPanel();
    serverVariablesPanel.setPreferredSize(new Dimension(588, 200));
    ...
    sl_topPanel.putConstraint(SpringLayout.NORTH, serverVariablesPanel, 7, SpringLayout.NORTH, variablesPanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, serverVariablesPanel, 6, SpringLayout.WEST, variablesPanel);
    sl_topPanel.putConstraint(SpringLayout.EAST, serverVariablesPanel, 6, SpringLayout.EAST, variablesPanel);
    sl_topPanel.putConstraint(SpringLayout.NORTH, linksPanel, 4, SpringLayout.SOUTH, serverVariablesPanel);
    sl_topPanel.putConstraint(SpringLayout.SOUTH, linksPanel, 7, SpringLayout.SOUTH, variablesPanel);
    sl_topPanel.putConstraint(SpringLayout.WEST, linksPanel, 6, SpringLayout.WEST, variablesPanel);
    sl_topPanel.putConstraint(SpringLayout.EAST, linksPanel, 6, SpringLayout.EAST, variablesPanel);
  ...
  private JPanel createServerVariablesPanel() {
    ...
    JScrollPane controlsScrollPane = new JScrollPane(controlsTable); //a scroll panel for the table
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    AbstractAction connectControlToVariable = new AbstractAction("Connect Writable to variable "){
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        String variable = org.colos.ejs.osejs.utils.EditorForVariables.edit(editor, "Variables", ":public|output", linksTable, varname, "");
    ...
    AbstractAction connectIndicatorToVariable = new AbstractAction("Connect Readable to Model Variable"){
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        // Passing an empty String as the last parameter hides the default methods "_isPlaying, _isPaused, _isApplet"         
        String variable = org.colos.ejs.osejs.utils.EditorForVariables.edit(editor, "Variables", ":public|input", linksTable, varname, "");
        if(variable != null) {
          if(variable.contains(":")) {
            String[] parts = variable.split(":");
            variable = parts[parts.length-1];
  ...
  private JScrollPane createLinksPanel() {
    ...
    linksScrollPane.setMinimumSize(new Dimension(600, 405));
    //linksScrollPane.setMaximumSize(new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE));
    linksScrollPane.setPreferredSize(new Dimension(600, 405));
  ...
  private JPanel createCodePanel() {
    ...
    topEvaluablePanel.setMinimumSize(new Dimension(600, 405));
    //topEvaluablePanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 405));
    topEvaluablePanel.setPreferredSize(new Dimension(600, 405));
